from flask import (
    render_template, flash, url_for, redirect, request, flash, session
)
...
from .forms import (
    LoginForm, RegistrationForm, ChangePasswordForm, ChangeEmailForm,
...
def login():
    ...
    return render_template('index.html',
        note_form=NoteForm(), login_form=form)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash('A confirmation email has bee sent to you by email.')
        return redirect(url_for('main.index'))
...
def change_email(token):
    if current_user.change_email(token):
        db.session.commit()
        flash('Your email address has been updated.', category='info')
    else:
